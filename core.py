...
class Password:
    def __init__(self, min_size=None, n_numb=None, n_alpha=None, n_char=None, not_numb=None, not_alpha=None,
        ...
        calc_percent = special_division(self._len) if min_size else None
        s_percent = calc_percent[0] if min_size else None
        t_percent = calc_percent[1] if min_size else None
        if n_numb:
            try:
                n_numb = int(n_numb)
                if n_numb < 0:
        ...
        match min_size:
            ...
            case _:
                self._numbers_a = n_alpha if n_alpha is not None else s_percent
    ...
    def with_alpha(self) -> list:
        ...
        match self._preferred_alpha.lower():
            ...
            case _:
                raise ValueError("The type of the letters must be uppercase (or upper), lowercase (or lower) or both "
                                 + "(all or default) !")
